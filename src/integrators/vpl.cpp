...
void VPLIntegrator::Preprocess(const Scene &scene, Sampler &sampler) {
  ...
  //           contrib *= _nLightPaths; // Actually, we can just pass in contrib in the next line, but we will keep this to remind
...
VPLIntegrator* CreateVPLIntegrator(const ParamSet& params, 
  ...
  int nLightPaths = params.FindOneInt("nlightpaths", 1000);
