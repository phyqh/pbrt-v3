...
void VARLMISLightSampler::Preprocess(const Scene& scene, 
...
Spectrum VARLMISLightSampler::Sample(const Vector3f& wo, const Interaction& it, const Scene& scene,
...

      {
        ...
        if (cutPdf > 0.0f && result.second > 0.0f && lightPdf > 0.0f) {
          ...
          reward = std::sqrt(reward * 
              ...
              result.second);
         LightTreeSamplingResult r1 = result;
         r1.first /= lightPdf;
          ...
          _hashMap[index]->samplingResults->Append(cutNode, r1);
          samplingRecords.push_back(SamplingRecord(cutNode, initTreeNode, result, reward));

          if (_biased) {
            Float curMaxQValue = lightCut->MaxWeight();
            lightCut->Update(cutNode, initTreeNode, [&](LightCutNode* node) {
              Float wt = node->weight;
              const LinearLightTreeNode* lightTreeNode = 
                _lightTree->getNodeByIndex(node->lightTreeNodeIndex);
...

  {
    ...
    for (const auto& samplingRecord : samplingRecords) {
      _hashMap[index]->lightCut->Update(samplingRecord.lightCutIndex, samplingRecord.lightTreeIndex, 
        ...
        wt = (1.0f - lr) * wt + lr * (samplingRecord.reward * misWeights[misIndex++] + _gamma * curMaxQValue);
        node->weight = wt;
      });
    }
  }
...
uint32_t VARLMISLightSampler::SampleAllocation(const LightCut* lightCut, uint32_t proposedSampleCount) const {
  if (!_isAdaptive) {
...
inline Float VARLMISLightSampler::ShadingPointEstimation(const LinearLightTreeNode* node) const {
  if (_isVpl) {
...
Spectrum VARLMISLightSampler::EstimateSingleLight(const Interaction &it, const Light &light, 
...
LightSampler* CreateVARLMISLightSampler(const ParamSet& params) {
  ...
  return new VARLMISLightSampler(directionalGridNumber, learningRate, gamma, 
      initLightCutSize, maxLightCutSize, isAdaptive, shadingPointClusters, biased);
}
