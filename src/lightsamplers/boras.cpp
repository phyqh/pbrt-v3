/*************************************************************************
    > File Name: src/lightsamplers/boras.cpp
    > Author: Yuchen Wang
...
void BayesianOnlineRegressionLightSampler::Preprocess(const Scene &scene, 
...
Spectrum BayesianOnlineRegressionLightSampler::Sample(const Vector3f &wi, const Interaction &it, 
        ...
        }
        {
          ...
          auto estimationFunction = [&](const std::shared_ptr<Light>& light) {
            ...
            if (lightPdf > 0 && !Li.IsBlack()) {
                ...
                if (it.IsSurfaceInteraction()) {
                    ...
                    value = Li.y() * AbsDot(wi, isect.shading.n);
          ...
          if (lightCutPdf > 0.0f && result.second > 0.0f && lightPdf > 0.0f) {
            records.push_back(LightSamplingRecord(lightCutNodeIndex, result.first, 
                  lightPdf * result.second * lightCutPdf, scatteringPdf, 
                  value / (result.second * lightPdf), SamplingMethod::BORAS));
      ...
      auto estimationFunction = [&](const std::shared_ptr<Light>& light) {
            ...
            if (lightPdf > 0 && !Li.IsBlack()) {
                ...
                if (it.IsSurfaceInteraction()) {
                    ...
                    f = isect.bsdf->f(isect.wo, wi, bsdfFlags) *
                        AbsDot(wi, isect.shading.n);
                    scatteringPdf = isect.bsdf->Pdf(isect.wo, wi, bsdfFlags);
                    vLight = Li.y() * AbsDot(wi, isect.shading.n);
                    VLOG(2) << "  surf f*dot :" << f << ", scatteringPdf: " << scatteringPdf;
                } else {
                    // Evaluate phase function for light sampling strategy
                    const MediumInteraction &mi = (const MediumInteraction &)it;
            ...
            if (!IsDeltaLight(light->flags)) {
                ...
                if (it.IsSurfaceInteraction()) {
                    ...
                    vBRDF = AbsDot(wi, isect.shading.n);
                ...
                if (!f.IsBlack() && scatteringPdf > 0) {
                    ...
                    if (foundSurfaceInteraction) {
                        ...
                        Li = light->Le(ray);
                    if (!Li.IsBlack()) {
                      vBRDF *= Spectrum(Li * Tr).y() * weight / scatteringPdf;
...
LightSampler* CreateBayesianOnlineRegressionLightSampler(const ParamSet& params) {
  ...
  return new BayesianOnlineRegressionLightSampler(shadingPointClusters, directionalGridNumber, 
